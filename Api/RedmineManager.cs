...
namespace Redmine.Net.Api
{
    ...
    public partial class RedmineManager
    {
        ...
        protected void LogTraceEvent(string message, EventLogEntryType type = EventLogEntryType.Information)
        {
        ...
        /// <param name="message">The message to log</param>
        /// <param name="type">The type of event</param>
        protected void LogErrorEvent(string message, EventLogEntryType type = EventLogEntryType.Error)
        {
            if (EventLog != null)
            {
                if (message.Length > 31000)
        ...
        private void ExecuteUpload(string address, string actionType, string data, string methodName)
        {
            using (var wc = CreateWebClient(null))
            {
                try
                {
                    ...
                    {
                        LogTraceEvent(String.Format("RedmineManager: address={0}, methodName={1}, data={2}", address, methodName, data), EventLogEntryType.Information);
                        wc.UploadString(address, actionType, data);
                    }
                }
                catch (WebException webException)
                {
                    LogErrorEvent(String.Format("RedmineManager: address={0}, methodName={1}, data={2}", address, methodName, data), EventLogEntryType.Error);
        ...
        private T ExecuteDownload<T>(string address, string methodName, NameValueCollection parameters = null) where T : class, new()
        {
            using (var wc = CreateWebClient(parameters))
            {
                ...
                {
                    LogErrorEvent(String.Format("ExecuteDownload: address={0}, methodName={1}, message={2}", address, methodName, webException.Message), EventLogEntryType.Error);
                    HandleWebException(webException, methodName);
                }
                return default(T);
            }
